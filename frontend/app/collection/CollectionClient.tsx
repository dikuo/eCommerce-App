...
import { useShop } from "@/context/ShopContext"; // 🟢 Using your custom hook
...
const CollectionClient = ({ initialProducts }: { initialProducts: Product[] }) => {
  ...
  useEffect(() => {
    ...
    if (subCategoryFromURL) setSubCategory([subCategoryFromURL]);
        ...
        {filterProducts.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 bg-zinc-50/50 rounded-3xl border border-dashed border-zinc-200">
            ...
            <button
              ...
              className="mt-4 text-sm font-bold text-black underline underline-offset-4 hover:text-zinc-600"
            >
              Clear all filters
            </button>
          </div>
